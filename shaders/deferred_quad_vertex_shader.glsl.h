...
inline std::string deferred_quad_vertex_shader_instanced = R"SHAD(
#version 460 core
...
layout(location = 4) in vec3 ipos; // the position variable has attribute position 0
...
void main()
{
    ...
    gl_Position = projection_matrix * view_matrix * (interpos + vec4(ipos,0.0));
    pos = (interpos + vec4(ipos, 0.0))/100.0;
...
inline std::string deferred_quad_vertex_shader = R"SHAD(
#version 460 core
layout(location = 0) in vec3 vpos; // the position variable has attribute position 0
layout(location = 1) in vec3 vnorm; // the position variable has attribute position 0
layout(location = 2) in vec2 vuv; // the position variable has attribute position 0
